        ...
        <div class="table-responsive">
            ...
            <div class="portlet blue box">
                <div class="portlet-title">
                    <a href="{{  url('/br-department/create') }}" class="btn btn-default pull-right" style="margin-top: 3px;">
                    ...
                    <table class="table table-striped table-bordered table-hover dt-responsive" width="100%" id="sample_5">
                        <thead>
                            <tr>
                                ...
                                <th class="min-phone-l">Division</th>
                        ...
                        <tbody>
                            <?php $i =0 ?> 
                            @foreach($br_department as $department)   
                            <?php $i++; ?>                                           
                            <tr>
                                <td>{{ $i }}</td>
                                <td>{{ $department->branch->branch_name ?? '' }}</td>
                                <td>{{ $department->division->br_name ?? '' }}</td>
                                <td>{{ $department->dept_name ?? '' }}</td>
                                <td>{{ $department->dept_details ?? '' }}</td>
                                <td>{{ ($department->dept_status == '1') ? 'Active':'Inactive' }}</td>
                                <td>
                                    <a href="{{ url('br-department/'.$department->id.'/edit') }}" class="btn btn-circle green btn-sm purple pull-left"><i class="fa fa-edit"></i>Edit</a>

                                    <form class="delete_item_form" accept-charset="UTF-8" action="br-department/{{ $department->id }}" method="POST"> 
                                    {!! csrf_field() !!} 
