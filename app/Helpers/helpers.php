...
function connectOracle(){
    //live
	//$conn = oci_connect('PAD_HRMS', 'PADMA_321HRM', '172.31.10.129:1599/orcl.padmabankbd.com');
...
function connectOracleOld(){
    //live
    //$conn = oci_connect('PAD_HRMS', 'PADMA_321HRM', '172.31.10.129:1599/orcl.padmabankbd.com');

    //Test
    //$conn = oci_connect('PAD_HRMS', 'PAD_HRMS', '192.168.200.135:1521/twodb');
    $conn = oci_connect('PAD_HRMS', 'padhrma', '192.168.200.16:1521/orcl1');
    if (!$conn) {
        $e = oci_error();
        trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
    }
