...
class AttendanceCronJob extends Command
{
     ...
     * @var string
     */
    protected $signature = 'AttendanceCronJob:attendanceCronJob';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $table_name = 'device_data';
             ...
             //$todays_attaend_data = \DB::connection('attendance_mysql')->table($table_name)->whereDate('date', $date)->where('is_retrive', '0')->whereBetween('date', [$from, $date]);
            // $todays_attaend_data = \DB::connection('attendance_mysql')->table($table_name)->whereBetween('date', [$from, $date])->where('is_retrive', '0')->take(10);
             $todays_attaend_data = \DB::connection('attendance_mysql')->table($table_name)->where('is_retrive',0);
            ...
            //$success_data = \DB::connection('attendance_mysql')->table($table_name)->where('is_retrive',1)->whereBetween('date', [$from, $date]);
            $success_data = \DB::connection('attendance_mysql')->table($table_name)->where('is_retrive',1);
